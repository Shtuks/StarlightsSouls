...
public class MutantTrueEyeR : ModProjectile
{
	...
	public override void AI()
	{
		...
		case 0:
		{
			Vector2 newVel = target.Center - base.Projectile.Center + new Vector2(300f * base.Projectile.localAI[1], -300f);
		...
		case 1:
			if (base.Projectile.localAI[0] == 1f)
			{
				for (int i = 0; i < 6; i++)
				{
					Vector2 spawnPos = base.Projectile.Center - Vector2.UnitY * 6f + new Vector2(100f, 0f).RotatedBy((float)Math.PI / 3f * (float)i);
					if (Main.netMode != 1)
					{
						Projectile.NewProjectile(Terraria.Entity.InheritSource(base.Projectile), spawnPos, Vector2.Zero, ModContent.ProjectileType<MutantTrueEyeSphere>(), base.Projectile.damage, 0f, base.Projectile.owner, (float)base.Projectile.identity, (float)i);
					}
				}
			}
		...
		case 2:
		{
			if (base.Projectile.localAI[0] == 1f)
			{
				...
				if (base.Projectile.velocity != Vector2.Zero)
				{
					...
					base.Projectile.velocity *= 24f;
				}
				base.Projectile.netUpdate = true;
			}
			else if (base.Projectile.localAI[0] > 10f)
			{
				base.Projectile.localAI[0] = 0f;
				base.Projectile.ai[1] += 1f;
			}
			float num3 = base.Projectile.velocity.ToRotation() + 1.570796f;
			if ((double)Math.Abs(base.Projectile.rotation - num3) >= 3.14159274101257)
			{
				base.Projectile.rotation = ((num3 >= base.Projectile.rotation) ? (base.Projectile.rotation + 6.283185f) : (base.Projectile.rotation - 6.283185f));
			}
			float num4 = 12f;
			base.Projectile.rotation = (base.Projectile.rotation * (num4 - 1f) + num3) / num4;
			break;
		}
		default:
		{
