...
public class MutantTrueEyeS : ModProjectile
{
	...
	public override void AI()
	{
		...
		case 0:
		{
			Vector2 newVel = target.Center - base.Projectile.Center + new Vector2(-200f * base.Projectile.localAI[1], -200f);
		...
		case 2:
			...
			else if (base.Projectile.localAI[0] == 14f)
			{
				this.ShootBolts(target);
			}
			else if (base.Projectile.localAI[0] > 21f)
			{
				base.Projectile.localAI[0] = 0f;
				base.Projectile.ai[1] += 1f;
			}
	...
	{
		Vector2 spawn = base.Projectile.Center - Vector2.UnitY * 6f;
		...
		if (vel != Vector2.Zero)
		{
			...
			if (Main.netMode != 1)
			{
				Projectile.NewProjectile(Terraria.Entity.InheritSource(base.Projectile), spawn, vel, 462, base.Projectile.damage, 0f, base.Projectile.owner, 0f, 0f);
			}
		}
	}
