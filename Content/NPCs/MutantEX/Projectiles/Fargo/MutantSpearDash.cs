...
public class MutantSpearDash : MutantSpearAttack
{
	...
	public override void AI()
	{
		...
		if (mutant.active && mutant.type == ModContent.NPCType<MutantEX>() && (mutant.ai[0] == 6f || mutant.ai[0] == 15f || mutant.ai[0] == 23f))
		{
			base.Projectile.velocity = Vector2.Normalize(mutant.velocity);
			base.Projectile.position -= base.Projectile.velocity;
			base.Projectile.rotation = mutant.velocity.ToRotation() + MathHelper.ToRadians(135f);
			base.Projectile.Center = mutant.Center + mutant.velocity;
			if ((base.Projectile.localAI[0] -= 1f) < 0f)
			{
				if (base.Projectile.ai[1] == -2f)
				{
					...
					for (int i = -1; i <= 1; i += 2)
					{
						if (Main.netMode != 1)
						{
							int p = Projectile.NewProjectile(Terraria.Entity.InheritSource(base.Projectile), base.Projectile.Center, 16f * Vector2.Normalize(mutant.velocity).RotatedBy((float)Math.PI / 2f * (float)i), ModContent.ProjectileType<MutantSphereSmall>(), base.Projectile.damage, 0f, base.Projectile.owner, -1f, 0f);
							if (p != 1000)
							{
				...
				else
				{
					...
					if (Main.netMode != 1)
					{
						Projectile.NewProjectile(Terraria.Entity.InheritSource(base.Projectile), base.Projectile.Center, Vector2.Zero, ModContent.ProjectileType<MutantSphereSmall>(), base.Projectile.damage, 0f, base.Projectile.owner, (float)mutant.target, 0f);
					}
				}
			}
		}
	...
	{
		Texture2D glow = ModContent.Request<Texture2D>("ssm/Assets/ExtraTextures/MutantEX/MutantEye_Glow", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value;
		int rect1 = glow.Height / Main.projFrames[base.Projectile.type];
		int rect2 = rect1 * base.Projectile.frame;
		Rectangle glowrectangle = new Rectangle(0, rect2, glow.Width, rect1);
		Vector2 gloworigin2 = glowrectangle.Size() / 2f;
		Color glowcolor = Color.Lerp(new Color(255, 55, 51, 0), Color.Transparent, 0.82f);
		Color glowcolor2 = Color.Lerp(new Color(255, 125, 132, 0), Color.Transparent, 0.6f);
		...
		float rotationModifier = 0f - MathHelper.ToRadians(135f) + (float)Math.PI / 2f;
		for (int i = 0; i < 3; i++)
		{
			...
			Main.EntitySpriteDraw(glow, drawCenter2 - Main.screenPosition + new Vector2(0f, base.Projectile.gfxOffY), (Rectangle?)glowrectangle, glowcolor, base.Projectile.rotation + rotationModifier, gloworigin2, scale * 1.25f, SpriteEffects.None, 0);
		}
		for (int i = ProjectileID.Sets.TrailCacheLength[base.Projectile.type] - 1; i > 0; i--)
		{
			...
			Main.EntitySpriteDraw(glow, value4 + base.Projectile.Size / 2f - Main.screenPosition + new Vector2(0f, base.Projectile.gfxOffY), (Rectangle?)glowrectangle, color27, base.Projectile.oldRot[i] + rotationModifier, gloworigin2, scale * 1.25f, SpriteEffects.None, 0);
		}
		return false;
	}
