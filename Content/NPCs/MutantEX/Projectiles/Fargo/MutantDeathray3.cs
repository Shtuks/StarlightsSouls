...
public class MutantDeathray3 : BaseDeathray, IPixelatedPrimitiveRenderer
{
	...
	public override void AI()
	{
		...
		float DECELERATION = 0.9716f;
		FargoSoulsUtil.NPCExists(base.Projectile.ai[1], ModContent.NPCType<MutantEX>());
		if (base.Projectile.localAI[0] == 0f)
		{
			...
			this.displayMaxTime = Math.Min(base.maxTime, base.Projectile.timeLeft + 2);
		}
		float num801 = 1f;
		...
		base.Projectile.scale = (float)Math.Sin(base.Projectile.localAI[0] * (float)Math.PI / this.displayMaxTime) * 6f * num801;
		if (base.Projectile.scale > num801)
		{
			base.Projectile.scale = num801;
		}
		float num804 = base.Projectile.velocity.ToRotation();
		if (base.Projectile.localAI[0] > 45f && base.Projectile.localAI[0] < base.maxTime - 120f)
		{
			base.Projectile.ai[0] *= DECELERATION;
		}
